...
// Places each sector at equal 60° steps around a circle, starting at 12 o'clock.
function orbitSlotStyle(index: number, radius: number): CSSProperties {
  const deg = index * 60 - 90;
  const rad = (deg * Math.PI) / 180;
  return { transform: `translate(${Math.sin(rad) * radius}px, ${-Math.cos(rad) * radius}px)` };
}
...
export function FuturePlansSection() {
  ...
  const [orbitRadius, setOrbitRadius] = useState(236);

  useEffect(() => {
    const update = () => {
      const w = window.innerWidth;
      if (w <= 760) return;
      setOrbitRadius(w <= 980 ? 198 : w <= 1180 ? 218 : 236);
    };
    update();
          ...
          <div className="future-hub">
            ...
            <div className="future-hub-orbit">
              {SECTORS.map((sector, index) => (
                <div key={sector.key} className="future-hub-slot" style={orbitSlotStyle(index, orbitRadius)}>
