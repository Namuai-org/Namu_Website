export const ha = {

  "nav.home":         "Game da Mu",
  "nav.products":     "Kayayyaki",
  "nav.models":       "Samfuranmu",
  "nav.mission":      "Manufarmu",
  "nav.contactSales": "Tuntubi tallace-tallace",
  "nav.tryFree":      "Gwada kyauta",

  "mission.quote":           "Fasaha na da ikon canza rayuwa, amma sai idan mutane za su iya amfani da ita da gaske. A Namu, muna gina tsarin da ke saduwa da mutane cikin harshensu da yanayinsu, domin damar rayuwa ta daina dogara kan inda mutum ya fito.",
  "mission.attributionLine": "Mouhamad Mamane, Wanda ya kafa Namu",
  "mission.statement1":      "Manufarmu ita ce mu sa AI ya zama mai saukin samu ga al'ummomin Afirka cikin harshe da al'adunsu.",
  "solution.step1.title": "Bayanan Magana, da Yarda",
  "solution.step1.body":  "Magana ta gaske, an tattara kuma an shirya tare da yardar al'ummomin da muke aiki da su.",
  "solution.step2.title": "Samfuran Asalin Magana",
  "solution.step2.body":  "An gina su domin fahimta da bada amsa kai-tsaye ta magana — ba a juya su daga rubutu ba.",
  "solution.step3.title": "Dandamali da APIs na Masu Haɓaka",
  "solution.step3.body":  "APIs, SDKs, da kayan aikin turawa masu sauki, domin kowace kungiya ta iya hada AI na asalin magana.",
  "solution.step4.title": "Kayayyaki da Haɗin Gwiwa",
  "solution.step4.body":  "Ana turawa ta makarantu, tsarin kiwon lafiya, gwamnatoci, da kamfanonin Afirka. Ginshiki da aikace-aikace, suna karfafa juna.",

  // ---- Shafin farko na edito ----------------------------------------------
  "nav.approach":  "Samfura",
  "nav.blog":      "Blog",
  "nav.brand":     "Alama",
  "nav.language":  "Harshe",

  "home.hero.line1": "Basira Mai",
  "home.hero.line2": "Jin Magana",

  "home.featured.alt":       "Alamar Namu da rubutun “Sanya AI ta yi aiki ga kowane harshe da kowace al'umma” bisa bishiyar kirya da dutse a faduwar rana",

  "home.film.kicker":      "Gajeren fim",
  "home.film.name":        "Fim din Namu",
  "home.film.intro":       "Dalilin da ya sa dole AI ta koyi sauraron mutanen da ba a taba gina ta dominsu ba, da kuma dalilin da ya sa muka fara da Hausa.",
  "home.film.description": "Gajeren fim cikin launukan Namu. A kan shafi mai launin kirim, rubutu ya ce AI na zama daya daga cikin kayan aiki mafi karfi a duniya, amma ga mutanen da ba sa rubutu, ko masu magana da harsunan da kwamfutoci ba su kula da su, ba a taba gina duniyar dijital domin ta saurare su ba. Shafin ya koma launin ruwan kasa, sai igiyar sauti ta bayyana: me zai faru idan AI za ta iya haduwa da mutane ta muryarsu? Fim din ya nuna Namu ta fara da Hausa, da bayanan magana da aka gina tare da al'ummomin da take yi wa hidima, da kuma iyawa uku: gane magana, samar da murya ta dabi'a, da fahimtar harshe. Ya kare cikin duhu da alamar Namu, a kan katunan da ke dauke da sunayen karin harsunan Afirka: ya kamata AI ta dace da mu, da muryoyinmu da harsunanmu.",

  "film.play":           "Kunna fim",
  "film.pause":          "Dakatar da fim",
  "film.replay":         "Sake kunnawa",
  "film.unmute":         "Kunna sauti",
  "film.mute":           "Kashe sauti",
  "film.fullscreen":     "Cikakken allo",
  "film.exitFullscreen": "Fita daga cikakken allo",
  "film.progress":       "Ci gaban fim",
  "film.of":             "cikin",
  "home.mission.statement": "A Namu muna kera samfuran murya ga harsunan Afirka da aka yi wa watsi da su, muna gina abubuwan more rayuwa da ake bukata don isar da su ta hanyoyin da kowa zai iya samu, muna kuma aiki tare da kungiyoyi domin amfani da fasahar don gina al'umma.",
  "home.mission.label":     "Manufarmu",
  "home.mission.expand":    "Gaba inda al'ummomin Afirka za su iya samu, su tsara, su kuma amfana daga Basirar Dan Adam ta Roba cikin harsunan da suke magana da rayuwar da suke ciki.",


  "home.models.title": "Gano ka kuma gwada samfuran muryarmu na zamani",
  "home.models.more":  "Kara sani",
  "home.models.all":   "Duk samfura",
  "models.intro":      "Kowane samfuri a nan an gina shi da magana tun farko, ga harsunan da masana'antar ta yi wa watsi. An horar da kowanne a kan rikodin da al'ummomin da suke magana da su suka bayar da yardarsu, an kuma tsara shi ya yi aiki a hanyoyin da mutane suka riga suka saba amfani da su.",
  "models.cta":        "Gwada samfuran",

  "home.model.interpret.kicker": "Fassarar magana",
  "home.model.interpret.name":   "Kora",
  "home.model.interpret.body":   "Hausa da Faransanci, a hanyoyi biyu cikin magana. Wani bangare ya yi magana, dayan ya ji da harshensa, tattaunawar kuma ta ci gaba da saurinta.",
  "home.model.interpret.alt":    "Zanen ruwa na kumfar magana biyu, daya mai alamar HA daya kuma mai alamar FR, hade da igiyoyin sauti",

  "home.model.haFr.kicker": "Fassarar magana",
  "home.model.haFr.name":   "Kora HA\u2192FR",
  "home.model.haFr.body":   "Ka yi magana ko ka yi tambaya da Hausa, ka ji amsa da Faransanci. Cikakkiyar tattaunawa cikin magana, ba sai ka rubuta ko ka karanta ba.",
  "home.model.haFr.alt":    "Zanen ruwa na kumfar magana biyu, daya mai alamar HA daya kuma mai alamar FR, hade da igiyoyin sauti",

  "home.model.frHa.kicker": "Fassarar magana",
  "home.model.frHa.name":   "Kora FR\u2192HA",
  "home.model.frHa.body":   "Wannan tattaunawa ta wata hanyar: ka yi magana ko ka yi tambaya da Faransanci, ka ji amsa da Hausa.",
  "home.model.frHa.alt":    "Zanen ruwa na kumfar magana biyu, daya mai alamar FR daya kuma mai alamar HA, hade da igiyoyin sauti",

  "home.model.asr.kicker":  "Ganewar magana",
  "home.model.asr.name":    "Tatsuniya",
  "home.model.asr.body":    "Mayar da maganar Hausa zuwa rubutu daidai, a kan lafuzza daban-daban, karin harshe, da wuraren hayaniya inda mutane ke yin rikodi.",
  "home.model.asr.alt":     "Zanen ruwa na igiyar sauti da ke koma layuka hudu na rubutu",

  "home.model.tts.kicker":  "Samar da magana",
  "home.model.tts.name":    "Tafiya",
  "home.model.tts.body":    "Maganar Hausa mai sauki da bayyana daga rubutu, an tsara ta domin a fahimce ta tun sau na farko.",
  "home.model.tts.alt":     "Zanen ruwa na kakaki mai fitar da sauti",

  "home.model.agent.kicker": "Wakilin murya",
  "home.model.agent.name":   "Aïr",
  "home.model.agent.body":   "Cikakkiyar tattaunawa da Hausa. Yana saurara, ya gane abin da kake bukata, sannan ya amsa da murya.",
  "home.model.agent.alt":    "Zanen ruwa na kumfar magana biyu da suka hadu, da igiyar sauti a tsakaninsu",

  "home.join.title": "Ku shiga tare da mu",
  "home.join.body":  "Mu karamar tawaga ce mai buri, ta masu bincike, injiniyoyi da masu gine-gine, muna aiki domin sanya AI ta kai ga al'ummomin Afirka.",
  "home.join.body2": "Za ka yi aiki a kan matsaloli masu wuya na magana, harshe, bayanai da samfuri, tare da taimakawa wajen kai ga miliyoyin mutanen da aka bar su a baya a juyin AI.",
  "home.join.body3": "Idan manufarmu ta burge ka, za mu yi farin cikin jin ta bakinka.",
  "home.join.cta":   "Tuntube mu",

  "home.langs.title":  "Inda muke farawa",
  "home.langs.intro":  "Zurfi kafin fadi. Harshe daya a yi shi yadda ya kamata, sannan na gaba.",
  "home.lang1.name":   "Hausa",
  "home.lang1.body":   "Daya daga cikin manyan al'ummomin harshe a Afirka, ana magana da ita a arewacin Najeriya da Nijar, kuma ita ce harshen da wadanda suka kafa Namu suka girma da shi. Nan ne ake kafa tushe.",
  "home.lang2.name":   "Zarma",
  "home.lang2.body":   "Ana magana da ita sosai a gefen kogin Niger a kudu maso yammacin Nijar. Ita ce ta gaba idan tushen Hausa ya yi karfi sosai.",
  "home.lang3.name":   "Fulfulde",
  "home.lang3.body":   "Ana magana da ita a fadin Sahel daga Senegal zuwa Kamaru, ta al'ummomin da suka saba wucewa tsakanin iyakoki da karin harshe.",

  "home.quote.name": "Mouhamad Mamane",
  "home.quote.role": "Wanda ya kafa kuma Shugaba na Namu",

  "footer.modelsTitle":  "Samfura",
  "footer.companyTitle": "Kamfani",
  "footer.investors":    "Masu zuba jari",
  "footer.legalTitle":   "Doka",
  "footer.contactTitle": "Tuntube mu",
  "footer.privacy":      "Sirri",
  "footer.terms":        "Sharudda",
  "footer.rights":       "© 2026 Namu. An kiyaye dukkan hakkoki.",

  // ---- Taken sama da ka'idoji ---------------------------------------------
  "home.hero.tagline": "AI cikin harsunan Afirka",

  "home.values.title": "Ka'idojin Mu",
  "home.value1.name": "Samun Dama",
  "home.value1.body": "Muna gina fasahar da mutane za su iya amfani da ita cikin sauki, ba tare da la'akari da harshensu, matakin karatunsu, inda suke, ko na'urar da suke amfani da ita ba.",
  "home.value2.name": "Girmamawa",
  "home.value2.body": "Muna kirkira tare da al'ummomi, ba kawai domin su ba. Harshensu, al'adunsu, sirrinsu, da abin da suka fuskanta a rayuwa suna tsara yadda muke gini.",
  "home.value3.name": "Amana",
  "home.value3.body": "Muna daukar daidaito, tsaro, yarda, da kariyar bayanai da muhimmanci. Ya kamata mutane da kungiyoyi su fahimci yadda tsarinmu ke aiki da inda iyakarsa take.",
  "home.value4.name": "Tasiri",
  "home.value4.body": "Muna mai da hankali kan sakamako mai amfani. Aikinmu ya kamata ya taimaka wa mutane su samu ilimi mai amfani, ya kuma taimaka wa kungiyoyi su yi wa al'ummomi hidima yadda ya kamata.",
  "home.value5.name": "Nauyi",
  "home.value5.body": "Muna daukar nauyin ingancin abin da muke ginawa, shawarwarin da muke yankewa, da sakamakon amfani da fasaharmu.",
  "home.value6.name": "Koyo",
  "home.value6.body": "Muna sauraro, muna gwada zatonmu, kuma muna ci gaba da ingantawa. Bincike, ra'ayin al'umma, da amfani a zahiri ya kamata su jagoranci hanyarmu.",

  // Menu na sama
  "nav.panel.models.title":   "Sabbin samfura",
  "nav.panel.models.body":    "Gano sabbin samfuran muryar Namu, wadanda aka gina domin yadda mutane ke magana da gaske.",
  "nav.panel.allModels":      "Duk samfura",
  "nav.panel.products.title": "Kayayyaki",
  "nav.panel.products.body":  "Kayan aiki masu jin magana ga mutane da kuma tawagogin da ke gini a kanmu.",
  "nav.panel.allProducts":    "Duk kayayyaki",
  "nav.panel.blog.title":     "Sabbin rubuce-rubuce",
  "nav.panel.blog.body":      "Rubuce-rubuce daga tawagarmu kan magana, harshe, da gina AI ga al'ummomin Afirka.",
  "nav.panel.allPosts":       "Duk rubuce-rubuce",
  "nav.product.studio.title": "Namu Studio",
  "nav.product.studio.body":  "Wurin aiki inda za ka iya magana, a kuma fahimce ka, cikin harshenka.",
  "nav.product.app.title":    "Manhajar Namu",
  "nav.product.app.body":     "Taimako na yau da kullum mai jin magana, a wayar da ke hannunka.",
  "nav.product.api.title":    "Namu API",
  "nav.product.api.body":     "Tsaftataccen API, SDK, da kayan turawa, don kowace tawaga ta iya hadawa da AI mai jin magana.",

  // ---- Playground ---------------------------------------------------------

  /* ---- The 3D playground -------------------------------------------------- */

  "park.eyebrow":           "Filin Wasan Namu",
  "park.title":             "Filin da za ka iya magana da shi.",
  "park.intro":             "Samfuran Namu guda huɗu, an gina su kamar abubuwan wasa da za ka gwada. Zaɓi ɗaya a ƙasa, ko ka danna sassaka.",
  "park.back":              "Duk filin",
  "park.classic":           "Salon gargajiya",
  "park.explore":           "Duba samfuran",
  "park.loading":           "Ana gina filin wasa",
  "park.hint.pointer":      "Ja don duba · Juya don kusantar · Danna sassaka don gwadawa",
  "park.hint.touch":        "Ja don duba · Matsa don kusantar · Taɓa sassaka don gwadawa",
  "park.hint.compact":      "Ja don duba · Taɓa sassaka",
  "park.station":           "Tasha",
  "park.hub.name":          "Blossom",
  "park.hub.how":           "Alamar Namu, an gina ta kamar ƙofa mai lanƙwasa. Tashoshi huɗu suna kewaye da ita.",
  "park.interpret.name":    "Kwanukan Raɗa",
  "park.interpret.how":     "Yi magana cikin kwanon Hausa. Faransanci zai fito daga kwanon da ke gaba.",
  "park.interpret.ha":      "Hausa",
  "park.interpret.fr":      "Français",
  "park.interpret.hint":    "yi magana nan",
  "park.transcribe.name":   "Ramin Saurare",
  "park.transcribe.how":    "Yi magana a bakin rami. Kalmominka za su fito a ɗaya gefen a matsayin baƙaƙe.",
  "park.voice.name":        "Ƙahon Magana",
  "park.voice.how":         "Rubuta a allo, sai ƙaho ya faɗe shi da ƙarfi a filin.",
  "park.voice.hint":        "rubuta wani abu nan",
  "park.agent.name":        "Bututun Zance",
  "park.agent.how":         "Yi magana cikin bututu ɗaya. Namu zai amsa daga ɗayan.",
  "park.agent.ask":         "Kai",
  "park.agent.answer":      "Namu",

  "playground.title":        "Filin gwaji",
  "playground.wordmark":     ":filin-gwaji",
  "playground.models":       "Samfura",
  "playground.sessions":     "Zamanka za su bayyana a nan",
  "playground.newSession":   "Sabon zama",
  "playground.explore":      "Duba samfuran",
  "playground.discover":     "An gina shi kan samfuran maganar Namu",
  "playground.park":         "Bude filin 3D",
  "playground.contact":      "Tuntubi tallace-tallace",
  "playground.collapse":     "Rufe gefen shafi",
  "playground.expand":       "Bude gefen shafi",
  "playground.disclaimer":   "Filin gwajin Namu sabo ne. Samfuran suna cikin koyo, kuma suna iya yin kuskure.",
  "playground.preview":      "Sabon gwaji. Samfuran na iya yin kuskure.",
  "playground.soon.label":   "Nan ba da jimawa ba",
  "playground.soon.title":   "Juzu'i na gaba yana kan hanya",
  "playground.soon.body":    "Muna kera juzu'i na biyu na wadannan samfura. Ku kasance tare da mu, ku dawo nan ba da jimawa ba don gwada su.",
  "playground.soon.follow":  "Bi ci gabanmu",
  "playground.attach":       "Hada sauti",
  "playground.record":       "Fara rikodi",
  "playground.stop":         "Tsayar da rikodi",
  "playground.send":         "Aika",
  "playground.cancel":       "Tsaya",
  "playground.clear":        "Share",
  "playground.running":      "Ana aiki",
  "playground.micDenied":    "Namu bai iya samun makarufonka ba. Duba izinin burauzar ka na wannan shafi.",
  "playground.voiceLabel":   "Murya",
  "playground.registerLabel":"Salo",
  "playground.dialectLabel": "Karin harshe",
  "playground.interpret.family": "Kora",
  "playground.dir.haFr":          "HA zuwa FR",
  "playground.dir.frHa":          "FR zuwa HA",
  "playground.directionLabel":    "Hanya",
  "playground.heard":        "Abin da aka ji",
  "playground.reply":        "Amsa",
  "playground.transcript":   "Rubutu",
  "playground.attached":     "An hada",
  "playground.pending.write":    "Ana samar da sauti",
  "playground.pending.listen":   "Ana rubutawa",
  "playground.pending.speak":    "Ana fassarawa",
  "playground.pending.converse": "Ana tunani",
  "playground.pending.slow1":    "Har yanzu ana aiki",
  "playground.pending.slow2":    "Yana daukar dan lokaci",
  "playground.pending.slow3":    "Ka dan jira",
  "playground.pending.sr":       "Har yanzu ana aiki. Dakiku {s} ya wuce.",
  "playground.copy":             "Kwafi",
  "playground.copied":           "An kwafa",
  "playground.download":         "Sauke",
  "playground.again":            "Sake gwadawa",

  "playground.haFr.title":       "Yi magana da Hausa, ji da Faransanci",
  "playground.haFr.placeholder": "Yi rikodi, ko ka hada sauti da Hausa",
  "playground.haFr.p1":          "Gaisuwar kasuwa",
  "playground.haFr.p2":          "Tambaya a asibiti",
  "playground.haFr.p3":          "Neman hanya",

  "playground.frHa.title":       "Yi magana da Faransanci, ji da Hausa",
  "playground.frHa.placeholder": "Yi rikodi, ko ka hada sauti da Faransanci",
  "playground.frHa.p1":          "Gaisuwar kasuwa",
  "playground.frHa.p2":          "Tambaya a asibiti",
  "playground.frHa.p3":          "Neman hanya",

  "playground.asr.title":       "Mayar da maganar Hausa zuwa rubutu",
  "playground.asr.placeholder": "Yi rikodi, ko ka hada sauti (.mp3, .wav) domin rubutawa",
  "playground.asr.p1":          "Labarin rediyo",
  "playground.asr.p2":          "Hira a fili",
  "playground.asr.p3":          "Sakon murya",

  "playground.tts.title":       "Daga rubutu zuwa magana, cikin dabi'a",
  "playground.tts.placeholder": "Rubuta abin da kake son ji",
  "playground.tts.p1":          "Sanarwar lafiya",
  "playground.tts.p2":          "Rahoton kasuwa",
  "playground.tts.p3":          "Farkon labari",

  "playground.agent.title":       "Tattaunawa, daga farko zuwa karshe",
  "playground.agent.placeholder": "Yi rikodi don fara magana",
  "playground.agent.p1":          "Tambaya kan yanayi",
  "playground.agent.p2":          "Yin alkawari",
  "playground.agent.p3":          "Neman hanya",

} as const;
